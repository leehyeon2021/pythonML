...
# 전체 데이터 중 Species가 'Iris-setosa'인 데이터와 'Iris-versicolor'인 데이터만 각각 추출하여 
# 별도의 데이터프레임(setosa_df, versicolor_df)에 저장하세요.
...
setosa_len = setosa_df['PetalLengthCm'].tolist()
setosa_wid = setosa_df['PetalWidthCm'].tolist()
versicolor_len = versicolor_df['PetalLengthCm'].tolist()
versicolor_wid = versicolor_df['PetalWidthCm'].tolist()

# [단계 4] 데이터 시각화
# 추출한 두 품종의 데이터를 산점도(Scatter plot)로 그리세요.
# X축: 꽃잎 길이(length), Y축: 꽃잎 너비(width)
# 두 품종의 색상을 다르게 표시하고 축 이름을 설정하세요.
plt.scatter( setosa_len, setosa_wid , c='purple')
plt.scatter( versicolor_len, versicolor_wid , c='violet')
plt.xlabel('꽃잎 길이(length)')
plt.xlabel('꽃잎 너비(width)')
plt.show()
...
length = setosa_len + versicolor_len
width = setosa_wid + versicolor_wid
